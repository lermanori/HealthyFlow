...
export default function HabitOutcomeSheet({ habit, date, onClose }: { habit: HabitItem; date: string; onClose: () => void }) {
  ...
  const remaining = target ? Math.max(0, target.value - info.progressTotal) : 0
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-end justify-center sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="habit-outcome-title">
      ...
      <div ref={panelRef} tabIndex={-1} className="relative flex max-h-[92dvh] w-full max-w-lg flex-col overflow-hidden rounded-t-3xl border border-line bg-page shadow-2xl outline-none sm:max-h-[88dvh] sm:rounded-3xl">
        ...
        {!isLoading && <div className="shrink-0 border-t border-line bg-page/95 px-5 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur-xl">
          {target ? <>
            <div className="grid grid-cols-2 gap-2"><button type="button" aria-label={info.outcome === 'completed' ? 'Completed' : `Log ${remaining} ${unitLabel(target.unit, remaining)} and finish`} disabled={mutation.isPending || info.outcome === 'completed'} onClick={() => submitTerminalOutcome('completed')} className="flex min-h-12 items-center justify-center gap-1.5 rounded-xl border border-emerald-400/35 bg-emerald-400/10 px-2 text-sm font-semibold leading-tight text-emerald-200 disabled:opacity-40"><Check className="h-4 w-4 shrink-0" />{info.outcome === 'completed' ? 'Completed' : <>Log {remaining} {unitLabel(target.unit, remaining)} &amp; finish</>}</button><button type="button" disabled={mutation.isPending || info.outcome === 'completed'} onClick={() => submitTerminalOutcome('failed')} className="flex min-h-12 items-center justify-center gap-1.5 rounded-xl border border-rose-400/35 bg-rose-400/10 px-2 text-sm font-semibold text-rose-200 disabled:opacity-40"><X className="h-4 w-4 shrink-0" />Not done</button></div>
            {info.outcome === 'failed' && <button type="button" onClick={() => mutation.mutate({ kind: 'outcome', outcome: 'pending' })} className="mt-2 flex min-h-11 w-full items-center justify-center gap-2 text-sm text-ink-muted"><RotateCcw className="h-4 w-4" />Clear outcome</button>}
          </> : <>
            <div className="grid grid-cols-2 gap-3"><button type="button" disabled={mutation.isPending} onClick={() => submitTerminalOutcome('completed')} className="flex min-h-14 items-center justify-center gap-2 rounded-2xl border border-emerald-400/35 bg-emerald-400/10 font-semibold text-emerald-200"><Check className="h-5 w-5" />Done</button><button type="button" disabled={mutation.isPending} onClick={() => submitTerminalOutcome('failed')} className="flex min-h-14 items-center justify-center gap-2 rounded-2xl border border-rose-400/35 bg-rose-400/10 font-semibold text-rose-200"><X className="h-5 w-5" />Not done</button></div>
            {info.outcome !== 'pending' && <button type="button" onClick={() => mutation.mutate({ kind: 'outcome', outcome: 'pending' })} className="mt-2 flex min-h-11 w-full items-center justify-center gap-2 text-sm text-ink-muted"><RotateCcw className="h-4 w-4" />Clear outcome</button>}
          </>}
        </div>}
      </div>
    </div>,
