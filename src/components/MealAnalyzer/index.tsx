...
export default function MealAnalyzer({ date, onClose }: MealAnalyzerProps) {
  ...
  return (
    <motion.div
      ...
      className="surface-overlay mx-auto flex w-full max-w-2xl flex-col overflow-hidden p-4 sm:p-6"
    >
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-ink">Add from a meal photo</h2>
            <p className="text-sm text-ink-soft">Describe a meal or snap a photo</p>
          </div>
        </div>
        {onClose && (
          <button onClick={onClose} className="rounded-lg p-2 text-ink-muted hover:bg-raised hover:text-ink-soft">
            <X className="h-5 w-5" />
          </button>
        )}
      </div>

      <div className="rounded-section border border-accent/40 bg-sunken/45 p-4">
        {photo && (
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-accent/10 pt-3">
          ...
          {photo && <ImageIcon className="h-4 w-4 text-accent" />}
          <button
            type="button"
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              ...
              className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-control bg-action text-on-action transition-colors hover:bg-action-hover disabled:cursor-not-allowed disabled:opacity-50"
      ...
      {suggestions.length > 0 && (
        <div className="mt-4 space-y-3">
          {review && (
            <div className="rounded-lg border border-state-warning/40 bg-state-warning/10 p-3 text-sm text-state-warning">
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  ...
                  {review.summary && <p className="mt-1 text-state-warning/80">{review.summary}</p>}
                  {review.reasons.length > 0 && (
                    <p className="mt-1 text-state-warning/80">{review.reasons.join(', ')}</p>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="grid max-h-72 gap-3 overflow-y-auto">
            {suggestions.map((meal, i) => (
              <div
                key={i}
                onClick={() => toggle(i)}
                className={`task-suggestion cursor-pointer ${selected.has(i) ? 'ring-2 ring-focus' : ''}`}
              >
                <div className="flex items-start space-x-3">
                  <div
                    className={`mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full border-2 transition-all ${
                      selected.has(i) ? 'border-action bg-action' : 'border-line-strong hover:border-accent'
                    }`}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <span className="rounded-full border border-accent/30 bg-accent/20 px-2 py-1 text-xs text-accent">
                        {meal.calories} cal
                      </span>
                      {meal.protein != null && (
                        <span className="rounded-full border border-line-strong/30 bg-raised/20 px-2 py-1 text-xs text-ink-soft">
                          P {meal.protein}g
                        </span>
                      )}
                      {meal.carbs != null && (
