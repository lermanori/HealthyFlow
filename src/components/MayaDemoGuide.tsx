...
export default function MayaDemoGuide() {
  ...
  return (
    <div className="fixed inset-0 z-[80] pointer-events-none">
      ...
      <div className="absolute inset-x-3 bottom-3 pointer-events-auto sm:bottom-4 lg:inset-x-auto lg:left-4 lg:top-24 lg:bottom-auto lg:w-[14rem] xl:w-[15.5rem]">
        <div className="mx-auto max-w-2xl rounded-xl border border-cyan-500/35 bg-page/90 p-3 shadow-2xl shadow-cyan-950/35 sm:p-4 lg:max-w-none">
          <div className="mb-3 flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-cyan-300">{step.eyebrow}</p>
              <h2 className="mt-1 text-lg font-bold leading-tight text-ink sm:text-xl lg:text-base">{step.title}</h2>
            </div>
            <button
              onClick={close}
              className="rounded-lg p-1.5 text-ink-muted transition hover:bg-card/70 hover:text-ink"
          ...
          <p className="text-sm leading-6 text-ink-soft lg:text-xs lg:leading-5">{step.subtitle}</p>

          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 lg:flex-col lg:items-stretch">
            ...
            <div className="flex items-center gap-2 lg:grid lg:grid-cols-[1fr_auto_auto]">
              <button
                onClick={() => setVoiceEnabled((value) => !value)}
                className="inline-flex items-center justify-center gap-2 rounded-lg border border-line/70 px-3 py-2 text-sm text-ink-muted transition hover:border-cyan-500/40 hover:text-cyan-200 lg:px-2 lg:text-xs"
              >
