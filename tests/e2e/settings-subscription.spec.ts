...
test('Settings exposes subscription and top-up contact flows', async ({ page }) => {
  ...
  await expect(page.getByText('Top up with 250 non-expiring credits for $5.')).toBeVisible()
  ...
  await page.getByRole('button', { name: 'Buy 250 · $5' }).first().click()
  await expect(page.getByRole('heading', { name: 'Buy more credits' })).toBeVisible()
  await expect(page.getByText('$5 for 250 non-expiring AI credits. Manual fulfillment for now.')).toBeVisible()
