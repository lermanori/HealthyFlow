...
session on iPhone, downloading and validating the account archive only when the
device has no matching document. Failure now leaves the session closed instead
of silently routing the account to hosted data; focused restoration tests, the
full frontend suite, production/Capacitor builds, and native Xcode build pass.
