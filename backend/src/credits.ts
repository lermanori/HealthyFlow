...
// a call cost us so `cost_usd` can be recorded and reported; since ADR-0016 they
...
// ─────────────────────────────────────────────────────────────────────────────
// SALE UNITS — what a user holds and spends. A credit is one action (ADR-0016).
//
 ...
 * That is the whole point of ADR-0016: the price does not move with actual usage,
...
function normalizeSubscriptionPricing(row: any, promoActiveOverride?: boolean): SubscriptionPricing {
  ...
  return {
    ...
    // — that rate is what drifted twenty-fold away from the meter before ADR-0016,
...
function subscriptionToClient(row: any) {
  return row ? {
    ...
    // Always zero since ADR-0016: Cloud includes text AI as an entitlement checked
...
export const Credits = {
  ...
  /**
   * Records what happened. Since ADR-0016 the price was fixed before the call, so
   ...
   * grants credits (ADR-0016). Callers are left in place so the admin surface keeps
   ...
   * twenty-fold from the cost meter before ADR-0016.
   */
