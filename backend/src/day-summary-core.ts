...
/**
 * The read seam shared by server-backed and device-backed days. Adapters own
 * persistence and return records; this module owns their meaning, degradation,
 * and composition into the one DaySummary contract.
 */
export type DaySummaryDependencies = {
  itemsForDay: (
    userId: string,
    date: string,
    timeZone?: string | null,
  ...
  listDayFocusBlocks: (
    userId: string,
    date: string,
  ) => Promise<DaySummary['work']['focusBlocks']>
}

export type DaySummarySourceError = {
  source: 'calendar_status' | 'calendar_events'
  userId: string
  date: string
  error: unknown
}

export type BuildDaySummaryCoreOptions = {
  now?: Date
  dependencies: DaySummaryDependencies
  onSourceError?: (failure: DaySummarySourceError) => void
}

export async function buildDaySummaryCore(
  userId: string,
  date: string,
  timeZone: string | null | undefined,
  options: BuildDaySummaryCoreOptions,
): Promise<DaySummary> {
...
export function validateDailyPlacementAgainstSummary(
  summary: DaySummary,
